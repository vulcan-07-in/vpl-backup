...
export async function fetchSquads(season: number = 1): Promise<Array<{ teamName: string; shortName: string; color: string; players: { name: string; role: string; price: string }[] }>> {
  ...
  if (season === 2) {
    ...
    const { data: players, error } = await supabase.from("player").select("name, role, price, team_id");
    if (error) {
      console.error("Supabase fetchSquads players error:", error);
      return [];
    }
    const teamMap = new Map<string, string>();
    teams.forEach(t => teamMap.set(t.teamName, t.teamName)); // placeholder if IDs differ
    const squads = teams.map(t => ({ ...t, players: [] as any[] }));
    const squadByName = Object.fromEntries(squads.map(s => [s.teamName, s]));
    (players || []).forEach(p => {
      const teamName = teamMap.get(p.team_id as string);
...
export async function fetchSquads(season: number = 1): Promise<Array<{ teamName: string; shortName: string; color: string; players: { name: string; role: string; price: string }[] }>> {
  ...
  if (season === 2) {
    // Get teams first
    const teams = await fetchTeams(2);
    // Get players (assuming player table has team_id referencing team.id)
    const { data: players, error } = await supabase.from("player").select("name, role, price, team_id");
    if (error) {
      console.error("Supabase fetchSquads players error:", error);
      return [];
    ...
    const teamMap = new Map<string, string>();
    teams.forEach(t => teamMap.set(t.teamName, t.teamName)); // placeholder mapping if IDs not available
    const squads = teams.map(t => ({ ...t, players: [] as any[] }));
    const squadByName = Object.fromEntries(squads.map(s => [s.teamName, s]));
    (players || []).forEach(p => {
      const teamName = teamMap.get(p.team_id as string);
